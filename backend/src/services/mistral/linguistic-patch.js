...
// Se almeno il 5% dei caratteri è cirillico, consideriamo il testo russo
// Abbassiamo la soglia per rendere più sensibile il rilevamento
const cyrillicRatio = totalChars > 0 ? cyrillicCount / totalChars : 0;
const isRussian = cyrillicCount > 0 && cyrillicRatio >= 0.05; // 5% invece di 10%
...
/**
* Ottiene un messaggio di risposta predefinito per fallback in russo
 * @returns {string} - Messaggio di risposta in russo
 */
...
return russianResponses[Math.floor(Math.random() * russianResponses.length)];
}
}
